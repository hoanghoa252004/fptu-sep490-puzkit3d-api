...
internal sealed class ActivateInstockPriceCommandHandler : ICommandHandler<ActivateInstockPriceCommand>
{
    private readonly IInstockPriceRepository _priceRepository;
    private readonly IInStockUnitOfWork _unitOfWork;

    public ActivateInstockPriceCommandHandler(
    ...
    public async Task<Result> Handle(ActivateInstockPriceCommand request, CancellationToken cancellationToken)
    {
        ...
        if (price.IsActive)
        {
            return Result.Failure(InstockPriceError.AlreadyActive(request.PriceId));
        }
